...
/*
 ...
 However, it also computes two integers, x and y, such that a*x + b*y = gcd(a,
 b).

 This is the preferred method for calculating the modular inverse of a number,
 as well as partitioning an integer into subgroups (as per the Chinese Remainder
 Theorem).
...
int main() {
  ...
  printf("gcd(%lld, %lld) = %lld = %lld * %lld + %lld * %lld\n", a, b,
         ret.first, a, ret.second.first, b, ret.second.second);
