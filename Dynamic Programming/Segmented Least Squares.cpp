...
/*
 Algorithm for calculating the minimal cost of approximating a set of points
 with segments

 The cost of a particular set of segments is calculated as sum(i=1 -> n) E_i +
 c*L, where E_i is the error of the i-th point WRT its segment, c the cost of
 each segment, and L the amount of segments used
...
double err[MAX_N][MAX_N];  // err[i][j] - total error by approximating points
                           // [i..j] by a segment
double a[MAX_N][MAX_N];  // a[i][j] - the slope of the segment used to
                         // approximate points [i..j]
double b[MAX_N][MAX_N];  // b[i][j] - the y-intercept of the segment used to
                         // approximate points [i..j]

int xySums[MAX_N], xSums[MAX_N], ySums[MAX_N], xSqrSums[MAX_N],
    ySqrSums[MAX_N];  // prefix sums for x_i * y_i, x_i, y_i, x_i * x_i and y_i
                      // * y_i

double minCost[MAX_N];  // minCost[j] - optimal cost for points [1..j]
int retIndex[MAX_N];  // the last segment in the optimal case for points [1..j]
                      // is [retIndex[j], j]
...
inline void Precalculate() {
  sort(pts + 1, pts + n + 1);  // can be omitted if points are guaranteed to be
                               // given in ascending X order
  ...
  for (int i = 1; i <= n; i++) {
    for (int j = i + 1; j <= n; j++) {
      ...
      a[i][j] = ((nn * xySum - xSum * ySum) * 1.0) /
                ((nn * xSqrSum - xSum * xSum) * 1.0);
      b[i][j] = ((ySum - a[i][j] * xSum) * 1.0) / (nn * 1.0);

      err[i][j] = a[i][j] * a[i][j] * xSqrSum + 2.0 * a[i][j] * b[i][j] * xSum -
                  2.0 * a[i][j] * xySum + nn * b[i][j] * b[i][j] -
                  2.0 * b[i][j] * ySum + ySqrSum;
    }
  }
}
...
inline void getSegments() {
  ...
  while (currInd > 1) {
    ...
    if (nextInd == currInd) {
      S.push(Segment(pts[currInd - 1].x, pts[currInd - 1].y, pts[currInd].x,
                     pts[currInd].y));
    } else {
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < ret.size(); i++) {
    printf("%d %d %d %d\n", (int)ret[i].x1, (int)ret[i].y1, (int)ret[i].x2,
           (int)ret[i].y2);
  }
  return 0;
}
