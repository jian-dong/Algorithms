...
/*
 ...
 character in a given set of characters, constructing an optimal lossless coding
 scheme with respect to the character distribution's entropy.

 The version implemented here analyses a given ASCII character string in order
 to obtain optimal codes based on character frequencies in the string. A more
 general version would receive a probability distribution instead.
...
inline void build_tree() {
  ...
  while (pq.size() > 1) {
    HuffTreeNode* L = pq.top();
    pq.pop();
    HuffTreeNode* R = pq.top();
    pq.pop();
