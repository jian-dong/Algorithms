...
/*
 Pairing Heap data structure satisfying the heap property, with highly efficient
 asymptotic bounds. Often outperforming Fibonacci heaps, and simple to
 implement. Complexity:    O(1) for insert, first and merge O(log n) amortized
 for extractMin and delete O(log n) amortized for decreaseKey - not known if
 bound is tight; Omega(log log N).
*/
...
bool PHeap::isEmpty() { return (this->root == NULL); }

void PHeap::insert(PNode *x) { this->merge(new PHeap(x)); }
...
PNode *PHeap::first() { return this->root; }
