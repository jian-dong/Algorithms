...
void BST::Insert(Node *x) {
  ...
  while (true) {
    if (x->key < curr->key) {
      ...
      return;  // Node with such key already exists in the tree
  }
}
  ...
  if (del->left == NULL && del->right == NULL)  // leaf
  {
    if (del->parent != NULL) {
      if (del->parent->left != NULL &&
          del->parent->left->root->key == del->key) {
        del->parent->left = NULL;
      } else if (del->parent->right != NULL &&
                 del->parent->right->root->key == del->key) {
...
void BST::prettyPrint() {
  ...
  // do a breadth-first traversal of the tree to get the depth and extract the
  // level by level node ordering. include non-existent nodes as empty space
  // keys.
  int depth = 0;
...
int main() {
  ...
  printf("Testing carried out on %d-%d-%d, by Petar Velickovic.",
  ...
  printf("Predecessor of 'D' is '%c'.",
         testRoot->predecessor(testRoot->search('D'))->key);
  printf("\n---------------------------------------------------------------\n");
  ...
  printf("Successor of 'R' is '%c'.",
         testRoot->successor(testRoot->search('R'))->key);
  printf("\n---------------------------------------------------------------\n");
