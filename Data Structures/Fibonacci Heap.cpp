...
/*
 Fibonacci Heap data structure satisfying the heap property, with highly
 efficient asymptotic bounds. Useful for a priority queue implementation in
 Dijkstra's and Prim's algorithms. Complexity:    O(1) for insert, first and
 merge O(1) amortized for decreaseKey O(log N) amortized for extractMin and
 delete
*/
...
bool FibHeap::isEmpty() { return (this->min == NULL); }

void FibHeap::insert(FibNode *n) { this->merge(new FibHeap(n)); }
...
FibNode *FibHeap::first() { return this->min; }
...
FibNode *FibHeap::extractMin() {
  ...
  if (ret->f == ret) {
    ...
    prev->f = next;
    next->b = prev;
    this->min = next;  // Not necessarily a minimum. This is for assisting with
                       // the merge w/ min's children.
  }
