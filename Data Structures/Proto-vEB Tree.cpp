...
/*
 ...
 Complexity:    O(log log u) for member
                O(log u) for insert, delete, min, max, extractMin, first and
 decreaseKey O(log u log log u) for pred, succ
*/
...
llu Proto_vEBTree::pred(llu x) {
  if (u == 2) {
    if (x == 1 && low)
      ...
      throw -1;  // Predecessor does not exist
  } else {
    llu subSize = (llu)sqrt(u);
    llu hi = x / subSize;
    llu lo = x % subSize;
...
llu Proto_vEBTree::succ(llu x) {
  if (u == 2) {
    if (x == 0 && high)
      ...
      throw -1;  // Successor does not exist
  } else {
    llu subSize = (llu)sqrt(u);
    llu hi = x / subSize;
    llu lo = x % subSize;
...
llu Proto_vEBTree::first() { return this->min(); }
