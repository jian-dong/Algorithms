...
class QueueOverflowException : public exception {
  virtual const char* what() const throw() { return "Error: Queue Overflow"; }
} qoverex;

class QueueUnderflowException : public exception {
  virtual const char* what() const throw() { return "Error: Queue Underflow"; }
} qunderex;

int BitQueue::size() { return this->valid_bits; }
...
int BitQueue::pop(int bsize) {
  ...
  int ret;
  if (bsize != 32)
    ret = ((this->queue) >> shift_amount) & ((1 << bsize) - 1);
  else
    ret = this->queue;
  this->valid_bits -= bsize;
  return ret;
