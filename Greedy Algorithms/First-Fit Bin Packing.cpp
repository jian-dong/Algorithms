...
/*
 The bin packing problem attempts to put n items of sizes 0 < s_i < 1 into bins
 of capacity 1. The first-fit algorithm utilises a greedy approach to the
 problem: it works by placing each item into the first bin it encounters that
 has enough free space (potentially creating a new bin if necessary). This
 implementation takes advantage of a segment tree data structure to quickly
 locate the first fit (assuming we know the number of items, n, in advance).
